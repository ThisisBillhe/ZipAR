    ...
    def forward(
        ...
        **kwargs,
        ...
        if past_key_value is not None:
            if local_position_ids is None:
                # sin and cos are specific to RoPE models; position_ids needed for the static cache
                cache_kwargs = {"sin": sin, "cos": cos, "cache_position": cache_position}
                key_states, value_states = past_key_value.update(key_states, value_states, self.layer_idx, cache_kwargs)
            else:
                past_keys = past_key_value.key_cache[self.layer_idx]
                past_values = past_key_value.value_cache[self.layer_idx]
                for i, pos in enumerate(local_position_ids):
                    if pos < past_keys.shape[2]:
                        past_keys = torch.cat((past_keys[:, :, :pos, :], key_states[:,:,i,:].unsqueeze(2), past_keys[:, :, pos:, :]), dim=2)
                        past_values = torch.cat((past_values[:, :, :pos, :], value_states[:,:,i,:].unsqueeze(2), past_values[:, :, pos:, :]), dim=2)
                    else:
                        assert pos == past_values.shape[2]
                        past_keys = torch.cat((past_keys, key_states[:,:,i,:].unsqueeze(2)), dim=2)
                        past_values = torch.cat((past_values, value_states[:,:,i,:].unsqueeze(2)), dim=2)

                past_key_value.key_cache[self.layer_idx] = past_keys
                past_key_value.value_cache[self.layer_idx] = past_values
                key_states = past_keys
                value_states = past_values
        ...
        # if attn_weights.shape[-1] == 147:
        #     torch.save(attn_weights.cpu(), '/mnt/workspace/Lumina-mGPT/saved_attns/attn_token{}.pth'.format(attn_weights.shape[-1]))
